...
const Main = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/allExperts")
      ...
      .then((data) => {
        ...
        const isExpert = data.find(
          (expert) =>
            expert.email === (loggedInUser.email || userLoggedInSession)
        )
        if (isExpert) {
          setIsExpert(true)
          setIsAdmin(false)
    ...
    fetch("http://localhost:5000/allAdmins")
      ...
      .then((data) => {
        ...
        const isAdmin = data.find(
          (admin) => admin.email === (loggedInUser.email || userLoggedInSession)
        )
        if (isAdmin) {
          setIsAdmin(true)
          setIsExpert(false)
  ...
  useEffect(() => {
    /** Prescription */
    fetch("http://localhost:5000/allPrescription")
      .then((response) => response.json())
      .then((data) => {
        setNumberPrescription(data)
        const prescriptionData = data.filter(
          (SinglePrescription) =>
            SinglePrescription.patientEmail ===
